"""
Edited version of PPO2 to support features used in multi-agent reinforcement learning
These include
...
class PPO_MA(ActorCriticRLModel):
    """
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            assert issubclass(self.policy, CnnLstmPolicy_MA), "Error: the input policy must be CnnLstmPolicy_MA"
            ...
            with self.graph.as_default():
                ...
                act_model = self.policy(self.sess, self.observation_space, self.action_space, self.n_envs,
                                        1, n_batch_step, n_players, n_roles, reuse=False, **self.policy_kwargs)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    tf.summary.scalar('role_prediction_loss', self.role_prediction_loss)
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, true_roles, train_mask,
        ...
        if writer is not None:
            ...
            if self.full_tensorboard_log and (1 + update) % 10 == 0:
                ...
                summary, policy_loss, value_loss, policy_entropy, approxkl, clipfrac, role_loss, _ = self.sess.run(
                    [self.summary, self.pg_loss, self.vf_loss, self.entropy, self.approxkl, self.clipfrac, self.role_prediction_loss, self._train],
            ...
            policy_loss, value_loss, policy_entropy, approxkl, clipfrac, role_loss, _ = self.sess.run(
                [self.pg_loss, self.vf_loss, self.entropy, self.approxkl, self.clipfrac, self.role_prediction_loss, self._train], td_map)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    ...
                    logger.logkv("ep_value", float(ep_value))
